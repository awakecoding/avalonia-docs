...
- A [`Style`](../../basics/user-interface/styling/index.md) is a CSS-like style. Styles aren't stored in a `Resources` collection as in WPF, they are stored in a separate `Styles` collection.
- A [`ControlTheme`](../../basics/user-interface/styling/control-themes.md) is similar to a WPF `Style` and is usually used to create themes for lookless controls
