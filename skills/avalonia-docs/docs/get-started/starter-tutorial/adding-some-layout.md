...
At this point, your temperature converter app has a single button in the center of the window. You cannot add any more elements, because each Avalonia window allows only one control in its content zone. (More on layout zones later, on the next page: [Customizing the Avalonia window](customizing-the-avalonia-window.md).)

To place multiple UI elements in the window, you must use a **layout control.**

For more information on layout controls, see the [Layout controls page](../../reference/controls/layout-controls.md).
