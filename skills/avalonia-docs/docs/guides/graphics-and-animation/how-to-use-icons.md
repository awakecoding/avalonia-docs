...
Path icons can draw icons from `Geometry` which includes using paths from the scalable vector graphics (SVG) format that can be customized with size and color. See the [reference](../../reference/controls/path-icon.md) for how to use this control.
